    ...
    return {
        ...
        generateEXDATEString: function(exdate, e){
            ...
            try {
                ...
                //Mojo.Log.error("Could not formate " + exdate._value);
                value = exdate._value;
            }
            ...
            try {
                Mojo.Log.info("Parsing " + value + " for exeption ts..");
                e.exdateTS = [];
                var dates = value.split(',');
                Mojo.Log.info("Got: " + JSON.stringify(dates));
                for (i = 0; i < dates.length; i++) {
                    try {
                        var ts = this.checkDate(value).getTime();
                        Mojo.Log.info("Got ts: " + ts);
                        e.exdateTS.push(ts);
                    } 
                    catch (error3) {
                        Mojo.Log.info("Expetion during TS generation: " + e + " - " + JSON.stringify(e));
                    }
                }
                Mojo.Log.info("TS array: " + JSON.stringify(e.exdateTS));
            } 
            catch (error4) {
                Mojo.Log.error("Expetion during exdate parsing: " + e + " - " + JSON.stringify(e));
            }
            ...
            if (rrule !== undefined && rrule !== null) {
                ...
                if (rrule.indexOf("UNTIL") !== -1) {
                    ...
                    try {
                        ...
                        Mojo.Log.error("Could not fill endValidity: " + e + " - " + JSON.stringify(e));
                        Mojo.Log.error("e: " + JSON.stringify(e));
                        Mojo.Log.error("RRULE: " + rrule + " until: " + until);
                    }
                }
                res += r;
        ...
        getTimestamp: function(date){
            if (date._utctimestamp) {
                Mojo.Log.error("Date had my UTC timestamp: " + date + " => " + date._utctimestamp);
                return date._utctimestamp;
            }
            else {
                Mojo.Log.error("No UTC timestamp set " + JSON.stringify(date) + "... might get into trouble here: " + date + " => " + date.getTime());
                return date.getTime();
            }
        },
        ...
        parseICalToEvent: function(icaltext){
            ...
            try {
                ...
                if (icalObj && icalObj.vtimezone && icalObj.vtimezone.tzid) {
                    ...
                    Mojo.Log.info("Got TZID " + tzid + " from vtimezone.");
                }
                else {
                    if (e.dtstart.tzid) {
                        tzid = e.dtstart.tzid;
                        Mojo.Log.info("Got TZID " + tzid + " from dtstart.");
                    }
                    else 
                        if (e.dtend.tzid) {
                            tzid = e.dtend.tzid;
                            Mojo.Log.info("Got TZID " + tzid + " from dtend.");
                        }
                }
                ...
                if (event.allday) {
					...
					Mojo.Log.error("Allday => Start: " + startDate + " End: " + endDate);
					...
					Mojo.Log.error("Allday Modified => Start: " + startDate + " End: " + endDate);					
                ...
                if (e.exdate !== undefined && e.exdate !== null && e.exdate !== "") {
                    Mojo.Log.info("Event has exdate: " + JSON.stringify(e.exdate));
                    ...
                    if (this.recurringEvents.length > 0) {
                        ...
                        for (i = 0; i < this.recurringEvents.length; i++) {
                            ...
                            Mojo.Log.info("Checking if event is exception for " + JSON.stringify(revent));
                            for (j = 0; j < revent.exdateTS.length; j++) {
                                var ts = revent.exdateTS[j];
                                Mojo.Log.info("Matching TS: " + ts + " = " + event.startTimestamp);
                                if (ts === tscleared) {
                                    event.originalStartTimestamp = revent.startTimestamp;
                                    event.parentId = revent.eventId;
                                    Mojo.Log.info("Found parent event with eventId " + revent.eventId + " ts: " + ts + " this ts " + event.startTimestamp);
                                    revent.exdateTS.splice(j, 1);
                                    if (revent.exdateTS.length === 0) //no exceptions left, remove event from recurring list.
                                    {
                                        Mojo.Log.info("All exceptions found for event..");
                ...
                Mojo.Log.error("Could not parse event. Error: " + exception);
				try{
					Mojo.Log.error(JSON.stringify(exception));
				}
				catch(exception2){ Mojo.Log.error(exception2 + " - " + JSON.stringify(exception2));}
                Mojo.Log.error(icaltext);
            }
        },
        ...
        makeICal: function(event){
            ...
            if (event.allday) {
				...
				Mojo.Log.error("Offset: " + startDate.getTimezoneOffset());
                Mojo.Log.error("Start: " + startTime + " Ende: " + endTime + " original was: " + new Date(event.startTimestamp) + " - " + new Date(event.endTimestamp));
            }
            ...
            if (value._value && value.tzid) {
                ...
                var offsetDate = function(date, tzid){
                    if (tzname === 'UTC') {
                        ...
                        Mojo.Log.error("Timezones: " + JSON.stringify(timezones) + " - " + tzid);
                        Mojo.Log.error("Get tz for: " + date.getDate() + "." + date.getMonth()+1 + "." + date.getFullYear() + " - " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds() + " in tz " + tzname);  
                        ...
                        Mojo.Log.error("Got timestamp " + timestamp + " for " + date);
