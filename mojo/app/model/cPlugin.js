var cPlugin = function(){
    ...
    return {
        ...
        sendSingle: function(r, event){
            ...
            var complete = "";
            if (event) {
                complete = iCal.makeICal(r);
            }
            else {
                complete += r;
            }
            if (complete.length > 250) {
                Mojo.Log.error("r is to long for a single send: " + complete);
        ...
        /**
         * This seems completed. But I recognized, that to long strings just get truncated.
         ...
         * ages anyway and won't be possible in background, at least not on my development Pixi Plus.
         *
        ...
        sendLoop: function(r, events){
            var i, c;
            if (!iCal.isArray(r)) {
                Mojo.Log.error("R is no array: " + r);
                this.sendSingle(r, events);
                return;
            }
            
            if (r.length === 0) {
                Mojo.Log.error("Empty loop. Will wake up c++!");
            ...
            for (i = 0; i < r.length; i++) {
                var complete = "";
                if (events) {
                    complete = iCal.makeICal(r[i]);
                }
                else {
                    complete += r[i];
                }
                //Mojo.Log.error("Write Event: " + complete + " is event: " + events);
                var length = complete.length;
                var chunks = Math.ceil(length / 250.0);
                var chunksSend = (events && r[i].eventId) ? chunks : chunks - 1;
                var offset = 0;
        ...
        forceReceive: function(string){
            try {
                ...
                Mojo.Log.error("Error received: " + e + " - " + JSON.stringify(e));
                Mojo.Log.error("Will call function again, soon.");
